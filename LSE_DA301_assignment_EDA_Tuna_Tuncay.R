...
# Finding Upper limit for outliers
EU_upper <-quantile(turtle_grp_product$Total_EU_Sales, probs=0.75) + 1.5 *IQR(turtle_grp_product$Total_EU_Sales)
NA_upper <-quantile(turtle_grp_product$Total_NA_Sales, probs=0.75) + 1.5 *IQR(turtle_grp_product$Total_NA_Sales)
Global_upper <-quantile(turtle_grp_product$Total_Global_Sales, probs=0.75) + 1.5 *IQR(turtle_grp_product$Total_Global_Sales)

# Filter outliers and check what products they are
filter(turtle_grp_product, Total_NA_Sales > NA_upper)
filter(turtle_grp_product, Total_EU_Sales > EU_upper)
filter(turtle_grp_product, Total_Global_Sales > Global_upper)
filter(turtle_grp_product, Total_NA_Sales > NA_upper & Total_Global_Sales > Global_upper & Total_EU_Sales > EU_upper)
...
# 4. Plot the data (with customer segments)
...
# Import librares to check performance
library(forecast)
install.packages('easystats')
library(easystats)

# Check for performance of the model
performance::check_model(model_multiple)
...
plot <- ggplot(predictall_df,aes(x=Actual_Global_Sales, y=Predict_Global_Sales, col=Diversion))+
  geom_point() +
  geom_smooth()
...
# When I plot actual and predicted values, it has almost a dioganal linear line.
# It means predicted values are very close to actual values. Model is working good.
###############################################################################
###############################################################################
